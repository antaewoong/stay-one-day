...
CREATE INDEX idx_web_sessions_session_id ON public.web_sessions(session_id);
CREATE INDEX idx_web_sessions_user_id ON public.web_sessions(user_id);
CREATE INDEX idx_web_sessions_created_at ON public.web_sessions(created_at);
CREATE INDEX idx_web_sessions_region_city ON public.web_sessions(region, city);
CREATE INDEX idx_web_sessions_utm_source ON public.web_sessions(utm_source);
CREATE INDEX idx_web_sessions_device_type ON public.web_sessions(device_type);
CREATE INDEX idx_web_sessions_converted ON public.web_sessions(converted) WHERE converted = true;

-- user_journey_events 테이블 인덱스
CREATE INDEX idx_user_journey_events_session_id ON public.user_journey_events(session_id);
CREATE INDEX idx_user_journey_events_user_id ON public.user_journey_events(user_id);
CREATE INDEX idx_user_journey_events_event_type ON public.user_journey_events(event_type);
CREATE INDEX idx_user_journey_events_page_path ON public.user_journey_events(page_path);
CREATE INDEX idx_user_journey_events_created_at ON public.user_journey_events(created_at);
CREATE INDEX idx_user_journey_events_conversion_type ON public.user_journey_events(conversion_type) WHERE conversion_type IS NOT NULL;

-- campaign_performance 테이블 인덱스
CREATE INDEX idx_campaign_performance_utm_source ON public.campaign_performance(utm_source);
CREATE INDEX idx_campaign_performance_date ON public.campaign_performance(date);
CREATE INDEX idx_campaign_performance_campaign_name ON public.campaign_performance(campaign_name);
...
CREATE POLICY "Admins can insert web sessions" ON public.web_sessions
    FOR INSERT WITH CHECK (true); -- 세션 추적은 누구나 가능

CREATE POLICY "Admins can update web sessions" ON public.web_sessions
    FOR UPDATE USING (true); -- 세션 업데이트는 누구나 가능

CREATE POLICY "Admins can view all user journey events" ON public.user_journey_events
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.admins
            WHERE user_id = auth.uid()
...
CREATE POLICY "Anyone can insert journey events" ON public.user_journey_events
    FOR INSERT WITH CHECK (true); -- 이벤트 추적은 누구나 가능
