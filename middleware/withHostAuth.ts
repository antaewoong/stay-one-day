...
const dbg = (...a: any[]) => console.log('[withHostAuth]', ...a)

export function withHostAuth(
  handler: (req: NextRequest, db: ReturnType<typeof createClient>, ctx: { userId: string; host: HostRow }) => Promise<NextResponse>
) {
  return async (req: NextRequest) => {
    ...
    // 2) 호스트 확인은 서비스키로
    const hostDB = createClient(URL, SERVICE, { auth: { persistSession: false } })
    const { data: host, error: herr } = await hostDB
      .from('hosts')
      .select('*')
      .eq('auth_user_id', user.id)
      .single<HostRow>()
    dbg('hostCheck', { ok: !!host, err: herr?.message ?? null })

    if (herr || !host) {
      return NextResponse.json({ ok: false, stage: 'hostCheck', error: 'Forbidden' }, { status: 403 })
    }

    return handler(req, db, { userId: user.id, host })
  }
}