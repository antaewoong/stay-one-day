...
export async function GET(req: Request) {
  ...
  if (!isJWT(token)) return Response.json({ ok:false, stage:'format', error:'invalid token format' }, { status:401 })

  const supa = createClient(URL, ANON, { auth:{ persistSession:false, autoRefreshToken:false } })
  const { data, error } = await supa.auth.getUser(token!)
  if (error || !data?.user) return Response.json({ ok:false, stage:'getUser', error: error?.message || 'no user' }, { status:401 })
