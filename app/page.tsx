...
export default function HomePage() {
  ...
  const [loading, setLoading] = useState(true) // 클라이언트에서 로드하므로 true로 시작
  ...
  useEffect(() => {
    ...
    const loadHomeData = async () => {
      ...
      try {
        ...
        // 2. 숙소 데이터 로드
        const accommodationResponse = await fetch('/api/accommodations?limit=1000')
        ...
        // 3. 사이트 섹션 설정 로드
        const sectionsResponse = await fetch('/api/site/sections')
        ...
        if (mounted) {
          ...
          setAllAccommodations(allAccommodations)
          setFeaturedStays(processedSections.featured)
          setPoolvillaStays(processedSections.poolvilla)
          setPrivateStays(processedSections.private)
          setKidsStays(processedSections.kids)
          setPartyStays(processedSections.party)
          setNewStays(processedSections.new)
          setLoading(false)
        }
