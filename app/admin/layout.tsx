...
export default function AdminLayout({
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      // sessionStorage에서 관리자 데이터 확인
      const adminUserData = sessionStorage.getItem('adminUser')
      
      console.log('Admin Layout 인증 체크:', { 
        hasAdminData: !!adminUserData 
      })
      
      if (!adminUserData) {
        console.log('Admin Layout: 인증 정보 없음, 로그인 페이지로 이동')
        setIsAuthenticated(false)
        router.replace('/admin/login')
      } else {
        console.log('Admin Layout: 인증 확인됨')
        setIsAuthenticated(true)
      }
    }
